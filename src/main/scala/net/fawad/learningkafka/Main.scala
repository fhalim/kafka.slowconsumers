...
object Main extends App {
  ...
  props.putAll(Map("bootstrap.servers" -> "localhost:9092", "group.id" -> "learningkafka", "max.partition.fetch.bytes"-> "1024"))
  val consumers = 5
  val sleepDuration = 60000
  ...
  for (producer <- managed(new KafkaProducer[String, String](props, new StringSerializer, new StringSerializer))) {
    val futures = for (c <- Range(0, consumers))
      yield Future {
        for (consumer <- managed(new KafkaConsumer[String, String](props, new StringDeserializer, new StringDeserializer))) {
          ...
          while (true) {
            val messageFromTopic = consumer.poll(1000)
    ...
    while(true){
      val producerSleep = sleepDuration/consumers/2
